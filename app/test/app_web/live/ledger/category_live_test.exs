defmodule AppWeb.Ledger.CategoryLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all categories", %{conn: conn, category: category} do
      {:ok, _index_live, html} = live(conn, ~p"/ledger/categories")
    ...
    test "saves new category", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/ledger/categories")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Category")
               |> render_click()
               |> follow_redirect(conn, ~p"/ledger/categories/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> render_submit()
               |> follow_redirect(conn, ~p"/ledger/categories")
    ...
    test "updates category in listing", %{conn: conn, category: category} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/ledger/categories/#{category}/edit")
  ...
  describe "Show" do
    ...
    test "displays category", %{conn: conn, category: category} do
      {:ok, _show_live, html} = live(conn, ~p"/ledger/categories/#{category}")
    ...
    test "updates category and returns to show", %{conn: conn, category: category} do
      {:ok, show_live, _html} = live(conn, ~p"/ledger/categories/#{category}")

      assert {:ok, form_live, _} =
               show_live
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/ledger/categories/#{category}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#category-form", category: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/ledger/categories/#{category}")
